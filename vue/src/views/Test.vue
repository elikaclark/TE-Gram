    ...
    <div class="row posts">
      <div class="col-sm-12 col-md-4 post-box" v-for="photo in photos" :key="photo.photo_id">
        ...
        <div>
          ...
          />
          <button type="button" class="btn btn-danger" v-if="!liked"
          v-on:click.prevent="addLikes(photo)">Like: {{photo.likes}}</button>
          <button type="button" class="btn btn-danger" v-else
          v-on:click.prevent="deleteLikes(photo)">Unlike: {{photo.likes}}</button>
